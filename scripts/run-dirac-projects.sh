...
run_one() {
  ...
  if [ -d "$wt" ]; then
    if [ -n "$(git -C "$wt" status --porcelain)" ]; then
      ...
      git -C "$wt" add -A
      # --no-verify: hooks (ruff format, db-docs-check) reformat or fail in worktree contexts
      # outside the docker-compose network; these are autonomous draft PRs that get human review.
      git -C "$wt" commit --no-verify -m "WIP: dirac autorun for $slug ($outcome)" >>"$LOG_DIR/$slug.log" 2>&1 || true
    fi
  ...
  if [ "$commits_ahead" -gt 0 ]; then
    ...
    if git -C "$wt" push --no-verify -u origin "$branch" >>"$LOG_DIR/$slug.log" 2>&1; then
