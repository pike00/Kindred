...
Connect Janet (the Hermes AI agent) to kindred (personal-crm at `kindred.khanpikehome.com`) via API-key auth so that:
...
Env vars needed:
- `KINDRED_API_URL` — e.g. `http://crm-api:8000/api/v1` (internal, on `pikenet-private`)
...
- Add `kindred-mcp` service on `pikenet-private` + `pikenet-internal-hermes`
- Mount `./kindred-mcp:/opt/kindred-mcp:ro` into hermes-agent
- Add `KINDRED_API_URL` and `KINDRED_API_KEY` env vars to hermes-agent
- Add hermes-agent to kindred's Docker network (or rely on `pikenet-private`)
...
POST /api/v1/webhooks/
{
  ...
  "url": "https://n8n.khanpikehome.com/webhook/<uuid>",
  "direction": "outbound",
