...
import { Users, Tag, FolderOpen, Bell, BookOpen, AlertTriangle, MessageCircle } from "lucide-react"

import useAuth from "@/hooks/useAuth"
import { ContactsService, TagsService, GroupsService, RemindersService, JournalService, InteractionsService } from "@/client"
            ...
            <div className="space-y-2">
              {recentInteractions.data.map((ix) => (
                <div key={ix.id} className="rounded-lg border bg-card p-3">
                  <div className="flex justify-between items-center">
                    <Badge variant="outline" className="text-xs">{ix.channel}</Badge>
                    <span className="text-xs text-muted-foreground">
                      {new Date(ix.occurred_at).toLocaleDateString()}
                    </span>
                  </div>
                  {ix.notes && (
                    <p className="text-sm text-muted-foreground mt-1 line-clamp-1">{ix.notes}</p>
                  )}
                </div>
              ))}
            </div>
