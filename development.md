...
For the homelab production deployment there is a separate `compose.prod.yml` that wires the stack into the `kindrednet-*` networks, Meilisearch, Redis, and the ARQ worker. Start it explicitly with `docker compose -f compose.prod.yml up -d`.
