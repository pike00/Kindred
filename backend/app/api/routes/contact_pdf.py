...
from fastapi import APIRouter, CurrentUser, Depends, HTTPException, Response
...
def _build_contact_one_pager_html(contact: ContactPublic, data: dict) -> str:
    ...
    if contact.avatar_url:
        avatar_html = f'<img class="avatar" src="{contact.avatar_url}" alt="{full_name}" />'
    else:
    ...
    if data.get("interactions"):
        interactions_html = '<div class="section"><h3>Recent Interactions</h3><ul class="timeline">'
        for ix in data["interactions"][:5]:
            ...
            notes = ix.notes[:200] + "..." if ix.notes and len(ix.notes) > 200 else (ix.notes or "")
            interactions_html += f'''
    ...
    if data.get("debts"):
        ...
        if active_debts:
            ...
            for d in active_debts:
                direction = "They owe me" if d.direction == DebtDirection.THEY_OWE else "I owe them"
    ...
    if data.get("relationships"):
        ...
        for rel in data["relationships"]:
            ...
            relationships_html += f'<li>{rel.relationship_type.title()}: {related_name}</li>'
    ...
    if data.get("notes"):
        ...
        for note in data["notes"][:3]:
            ...
            notes_html += f'<div class="note"><p>{body}</p><small>{created}</small></div>'
        notes_html += '</div>'
    ...
    if contact.how_we_met:
        how_met_html = f'<div class="section"><h3>How We Met</h3><p>{contact.how_we_met}</p></div>'
...
async def get_contact_pdf(
    ...
    # Gifts
    gifts = session.exec(
        select(Gift).where(Gift.contact_id == contact_id)
    ).all()
    data["gifts"] = gifts
    ...
    for rel in relationships:
        rel_dict = {"relationship_type": rel.relationship_type, "related_contact_name": None}
        related = session.get(Contact, rel.related_contact_id)
        if related and related.id in visible_ids:
            rel_dict["related_contact_name"] = f"{related.first_name} {related.last_name or ''}".strip()
