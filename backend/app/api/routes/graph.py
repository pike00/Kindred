...
def get_contacts_graph(
    ...
    if root_contact_id:
        ...
        visited = {root_contact_id}
        frontier = {root_contact_id}
        current_depth = 0
        ...
        while current_depth < depth and frontier:
            ...
            rels_stmt = select(Relationship).where(
                Relationship.contact_id.in_(frontier)
            )
            relationships = session.exec(rels_stmt).all()
        ...
        contacts_stmt = select(Contact).where(Contact.id.in_(visited))
        contacts = session.exec(contacts_stmt).all()
        ...
        edges_stmt = select(Relationship).where(
            Relationship.contact_id.in_(visited),
            Relationship.related_contact_id.in_(visited),
        )
        relationships = session.exec(edges_stmt).all()
