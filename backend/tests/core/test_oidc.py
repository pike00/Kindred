...
def _cf_env(monkeypatch):
    monkeypatch.setattr(
        oidc.settings, "OIDC_ISSUER_URL", "https://team.cloudflareaccess.com"
    )
...
def test_verify_success(rsa_key, monkeypatch):
    ...
    token = _make_token(
        rsa_key, aud="aud-hash-123", iss="https://team.cloudflareaccess.com"
    )
...
def test_wrong_audience(rsa_key, monkeypatch):
    ...
    oidc._reset_cache_for_tests()
    token = _make_token(
        rsa_key, aud="other-aud", iss="https://team.cloudflareaccess.com"
    )
    with patch("app.core.oidc._fetch_jwks", return_value=_jwks(rsa_key)):
...
def test_wrong_issuer(rsa_key, monkeypatch):
    _cf_env(monkeypatch)
    oidc._reset_cache_for_tests()
    token = _make_token(rsa_key, aud="aud-hash-123", iss="https://attacker.example.com")
    with patch("app.core.oidc._fetch_jwks", return_value=_jwks(rsa_key)):
        with pytest.raises(oidc.OIDCError):
            oidc.verify_oidc_token(token)
